...
// Sports from the dedicated getLiveSports API (excludes the "all sports" entry)
const liveSports = computed(() => {
  return sports.value.filter((s) => s.sportId !== "");
});

// Market tabs — hardcoded common markets
const marketTabs = [
  { label: "1X2", index: 0 },
...
<template>
  <section class="mt-3">
    <!-- Sticky header -->
    <div class="sticky top-30 z-40 live-preview-card mx-3 rounded-t-xl">
      ...
      <div class="flex items-center gap-3 px-3 pt-3 pb-1.5 overflow-x-auto scrollbar-hide">
        ...
        <div class="flex items-center gap-3">
          <button
            ...
            :key="sport.sportId"
            class="text-[0.75rem] font-semibold whitespace-nowrap shrink-0 transition-colors"
            :class="[
      ...
      <div class="flex items-center gap-2 px-3 pb-2 overflow-x-auto scrollbar-hide">
        <button
          ...
          :key="tab.index"
          class="shrink-0 px-3 py-1 rounded-full text-[0.7rem] font-semibold whitespace-nowrap transition-colors"
          :class="[
    ...
    <!-- Content -->
    <div class="live-preview-card mx-3 rounded-b-xl overflow-hidden">

      <!-- Loading skeleton -->
      <div v-if="previewPending" class="space-y-px">
        <div v-for="i in 3" :key="i" class="px-3 py-4">
          <div class="h-3 w-3/4 bg-gray-200 dark:bg-white/5 rounded animate-pulse mb-3"></div>
          <div class="flex items-center gap-3">
            <div class="w-[55%] space-y-1.5">
              <div class="h-3 w-4/5 bg-gray-200 dark:bg-white/5 rounded animate-pulse"></div>
              <div class="h-3 w-3/5 bg-gray-200 dark:bg-white/5 rounded animate-pulse"></div>
            </div>
            <div class="w-[45%] grid grid-cols-3 gap-1.5">
              <div class="h-8 bg-gray-200 dark:bg-white/5 rounded-md animate-pulse"></div>
              <div class="h-8 bg-gray-200 dark:bg-white/5 rounded-md animate-pulse"></div>
              <div class="h-8 bg-gray-200 dark:bg-white/5 rounded-md animate-pulse"></div>
            </div>
          </div>
      ...
      <!-- Match rows -->
      <div v-else-if="flatMatches.length > 0" class="divide-y divide-gray-200/70 dark:divide-white/5">
        <div
        ...
        >
          ...
          >
            ...
            <span class="text-[0.7rem] font-semibold text-gray-700 dark:text-white shrink-0">
              {{ match.statusDesc || "" }}
            </span>
            <span class="text-[0.65rem] text-gray-400 dark:text-white/40 truncate min-w-0">
          ...
          <div class="flex items-center">
            ...
            >
              <div class="flex items-center justify-between">
                <span class="text-[0.8rem] font-medium text-gray-800 dark:text-white/85 truncate mr-2">
                  {{ match.homeTeam }}
                </span>
            ...
            <div class="w-[45%]">
              ...
              >
                <button
                  ...
                  :key="i"
                  class="locked-btn flex flex-col justify-center items-center p-1 h-full min-h-10 rounded-md transition-colors opacity-50"
                  @click="goToMatchDetails(match, router, true)"
                >
      ...
      <!-- Footer: All Live Events -->
      <div class="live-preview-footer px-3 py-2.5">
        <RouterLink
...
[data-theme="dark"] .live-preview-card {
  background: var(--card);
...
[data-theme="dark"] .live-preview-footer {
...
[data-theme="dark"] .locked-btn {
  background: oklch(100% 0 0 / 0.05);
}
.locked-btn:hover {
  background: oklch(0% 0 0 / 0.06);
}
[data-theme="dark"] .locked-btn:hover {
