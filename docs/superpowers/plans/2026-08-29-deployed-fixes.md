...
### Task 3 applied (2026-08-29, commit `af4ba1f`, user decision "do it")
`nitro.vercel.functions.regions = ["cpt1"]` + `vercel.json` `regions`. Verified in the Vercel-preset build (`.vercel/output/functions/__fallback.func/.vc-config.json` → `"regions": ["cpt1"]`) and live: `x-vercel-id` went from `cpt1::iad1` to **`cpt1::cpt1`**. TTFB from Kenya (5 cache-busted samples): **0.29–0.61 s, median ≈ 0.30 s** (before: 0.53–0.75 s, median ≈ 0.74 s); whole HTML response ≈ 0.36–0.69 s (before ≈ 0.77–1.18 s). Lighthouse from this vantage remains noisy run-to-run (71 / 84 / 90 on consecutive runs) — the deterministic gain is the ~0.4 s off every HTML response for African users. Note: Nitro 2.13's `vercel.regions` only applies to the edge preset; the serverless preset spreads `vercel.functions` into `.vc-config.json`, which is why `regions` lives there.
